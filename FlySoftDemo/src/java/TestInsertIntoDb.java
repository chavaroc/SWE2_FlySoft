...
/**
 * This class performs a data download from trans stats (via crawler), parse the
 * data and write them into db.
 *
...
public class TestInsertIntoDb {
    ...
    public TestInsertIntoDb(List<String> fileList) {
    ...
    private void writeIntoDB() throws Exception {
         ...
         * onTimeTableFile; List<String> fileList = new ArrayList<String>();
         ...
         * EnumSet.of(CrawlTableType.OnTime)); crawler.crawl();
         * fileList.addAll(crawler.getCrawledFileNames());
         *
         * //if(args.length > 0){ // filePath = args[0]; //} //else{ //
         * System.out.println("Required OnTime csv file path as first
         * parameter."); //return; //} for(String filePath : fileList){ File f =
         * new File(filePath); if(!f.exists()){ System.out.println("Skipped file
         * '"+filePath+"' because it does not exist"); continue; }
         * System.out.println("Parse file '"+filePath+"'");
         *
         * onTimeTableFile = new File(filePath); AbstractMapTable config =
         * OnTimeMapTable.getInstance(); CsvParser<ParsedFlight> parser = new
         * CsvParser<>(onTimeTableFile.getAbsolutePath(), config, ',',
         * ParsedFlight.class);
         *
         * System.out.println("Start parsing..."); List<ParsedFlight>
         * parsedFlights = parser.parse();
         * System.out.println(parsedFlights.size() + " flights parsed.");
         *
         * System.out.println("Start filtering..."); NewYorkFlightFilter
         * newYorkFilter = new NewYorkFlightFilter(); parsedFlights =
         * newYorkFilter.filter(parsedFlights); System.out.println("Filtering
         * finished ("+parsedFlights.size() +" flights left).");
         *
         * System.out.println("Prepare flights."); FlightOnTimePreparator
         * preparator = new FlightOnTimePreparator();
         * preparator.prepareAll(parsedFlights); System.out.println("Preparation
         * finished.");
         *
         * System.out.println("Start adding to database...");
         * ParsedFlightController controller = new ParsedFlightController();
         * controller.createAll(parsedFlights); System.out.println("Insertion to
         * database finished.");
        }
         */
    }
